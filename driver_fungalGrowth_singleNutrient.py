...
#cwd_path ='D:/Fungi_new_v80'
cwd_path ='bigdata/alberlab/kvo020/Hyphae_NoFusion_12'
...
def driver_singleNutrient(run):
    ...
    # DualLogger class for simultaneous console and file logging
    class DualLogger:
    ...
    # Get parameter string early for log file path
    folder_string_temp, param_string_temp = hf.get_filepath(params)
    
    # Create log directory
    log_dir = f"Results/{param_string_temp}/Run{run}"
    os.makedirs(log_dir, exist_ok=True)
    log_file_path = f"{log_dir}/simulation_log_run{run}.txt"
    ...
    print("=" * 80)
    print(f"=== Fungal Growth Simulation Run {run} ===")
    print(f"Parameter set: {param_string_temp}")
    ...
    try:
        ...
        # Is the nutrient background variable or fixed? If fixed, then the nutrient concentration in the environment
        ...
        if (isPatchyExtEnvironment == 0):
            ...
            x_vals, y_vals, sub_e_gluc, sub_e_treha, glucose_mask = sf.external_grid_patchy(setBackground, params, run+1)
        ...
        count_times = []
        count_biomass = []
        count_longest_branch = []
        count_nutrients_consumed = []
        total_length_progression = []
        initial_total_nutrients = np.sum(sub_e_gluc)  # Store initial nutrient amount for reference
        ...
        # Calculate initial mass state for conservation tracking
        initial_mass_dict = hf.calculate_total_mass_with_metabolites(
            mycelia, num_total_segs, sub_e_gluc, sub_e_treha, params)
        hf.print_initial_mass_state(initial_mass_dict, params)
        
        # Initialize diffusion conservation tracking
        diffusion_check_count = 0
        diffusion_fail_count = 0
        max_diffusion_error = 0.0
        ...
        while current_time < params['final_time']: 
            ...
            while(this_time < ntimes):
                ...
                if current_step % step_size_extern == 0:  
                    ...
                    if (glucDiff != 0):
                        ...
                        # Store pre-diffusion state for conservation check
                        sub_e_gluc_before = sub_e_gluc.copy()
                    
                        # Perform diffusion
                        sub_e_gluc_new = nf.finite_volume_diffusion_2d(sub_e_gluc, params['dt_e'], params, glucose_mask)
                        sub_e_gluc[glucose_mask] = sub_e_gluc_new[glucose_mask]
                    
                        # Check diffusion conservation
                        is_conserved, diff_report = hf.check_diffusion_conservation(
                            sub_e_gluc_before, sub_e_gluc, glucose_mask, 
                            tolerance=1e-10
                        )
                        diffusion_check_count += 1
                        if not is_conserved:
                            diffusion_fail_count += 1
                            hf.print_diffusion_conservation_warning(diff_report)
                        # Track maximum error
                        if diff_report['relative_error'] > max_diffusion_error:
                            max_diffusion_error = diff_report['relative_error']
            ...
            # if current_step % (1*160) == 0:
                ...
                # Calculate longest branch length
                longest_branch_length = hf.calculate_longest_branch(mycelia, num_total_segs)
                count_longest_branch.append(longest_branch_length)
            
                # Calculate nutrients consumed
                nutrients_consumed = (initial_total_nutrients - np.sum(sub_e_gluc))
                count_nutrients_consumed.append(nutrients_consumed)
            
                # Check mass conservation
                current_mass_dict = hf.calculate_total_mass_with_metabolites(
                    mycelia, num_total_segs, sub_e_gluc, sub_e_treha, params)
                conservation_check = hf.check_mass_conservation(
                    initial_mass_dict, current_mass_dict)
                hf.print_mass_conservation_report(conservation_check, current_time, params)
            
                # Print diffusion conservation statistics
                if diffusion_check_count > 0:
                    success_rate = ((diffusion_check_count - diffusion_fail_count) / 
                                   diffusion_check_count * 100)
                    print("\n" + "="*80)
                    print("DIFFUSION CONSERVATION STATISTICS")
                    print("="*80)
                    print(f"Total diffusion steps checked: {diffusion_check_count}")
                    print(f"Failed checks: {diffusion_fail_count}")
                    print(f"Success rate: {success_rate:.2f}%")
                    print(f"Maximum relative error: {max_diffusion_error:.2e}")
                    print("="*80 + "\n")
        ...
        # Calculate final longest branch length
        longest_branch_length = hf.calculate_longest_branch(mycelia, num_total_segs)
        count_longest_branch.append(longest_branch_length)
    
        # Calculate final nutrients consumed
        nutrients_consumed = (initial_total_nutrients - np.sum(sub_e_gluc))
        count_nutrients_consumed.append(nutrients_consumed)
        ...
        count_times = np.array(count_times)
        count_biomass = np.array(count_biomass)
        count_longest_branch = np.array(count_longest_branch)
        count_nutrients_consumed = np.array(count_nutrients_consumed)
        avg_treha_annulus = np.array(avg_treha_annulus)
        max_treha_annulus = np.array(max_treha_annulus)
        min_treha_annulus = np.array(min_treha_annulus)
       ...
       # Print out results:
        ...
        # Print final diffusion conservation summary
        if diffusion_check_count > 0:
            success_rate = ((diffusion_check_count - diffusion_fail_count) / 
                           diffusion_check_count * 100)
            print('\n' + '='*80)
            ...
            # Status assessment
            if diffusion_fail_count == 0:
                status = "EXCELLENT - Perfect conservation"
            elif success_rate >= 99.9:
                status = "ACCEPTABLE - Minor numerical errors"
            elif success_rate >= 95.0:
                status = "WARNING - Frequent numerical errors"
            else:
                status = "ERROR - Significant conservation violations"
        ...
        output_dict = {
            ...
            'array_times' : count_times,
            'total_biomass' : count_biomass,
            'longest_branch_length' : count_longest_branch,
            'nutrients_consumed' : count_nutrients_consumed,
            'max_radii' : count_max_radii,
            'avg_radii' : count_avg_radii,
            'rms_radii' : count_rms_radii,
            'rms_tip_radii' : count_rms_tip_radii,
        ...
        hf.plot_biomass_vs_time(np.array(count_times), np.array(count_biomass), 
                               folder_string, param_string, params, run)
    
        # Plot longest branch vs time
        hf.plot_longest_branch_vs_time(np.array(count_times), np.array(count_longest_branch),
                                       folder_string, param_string, params, run)
        
            # Plot biomass vs nutrients consumed
        hf.plot_biomass_vs_nutrients_consumed(np.array(count_biomass), np.array(count_nutrients_consumed),
                                             folder_string, param_string, params, run)
        
        # Plot longest branch vs nutrients consumed
        hf.plot_longest_branch_vs_nutrients_consumed(np.array(count_longest_branch), np.array(count_nutrients_consumed),
                                                    folder_string, param_string, params, run)
        
        # Print simulation completion
        time_total = time.time() - t_0
        time_mins = int(time_total // 60)
        time_secs = time_total % 60
        print("\n" + "=" * 80)
        print(f"=== Simulation Run {run} Completed Successfully ===")
        print(f"End time: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Total runtime: {time_total:.2f} seconds ({time_mins} mins, {time_secs:.2f} secs)")
        print("=" * 80)
        
    except Exception as e:
        print("\n" + "!" * 80)
        print(f"!!! ERROR in Simulation Run {run} !!!")
        print(f"Exception: {e}")
        print("!" * 80)
        import traceback
        traceback.print_exc()
        ...
        logger.close()
        print(f"[Main Process] Log saved to: {log_file_path}")
...
all_radii = np.array([])
all_biomass = np.array([])
all_longest_branch = np.array([])
all_nutrients_consumed = np.array([])
...
if num_runs > 1:
    ...
    for run in range(num_runs):
        if run == 0:
            ...
            all_biomass = np.hstack((all_biomass, output_dict[run]['total_biomass'].flatten()))
            all_longest_branch = np.hstack((all_longest_branch, output_dict[run]['longest_branch_length'].flatten()))
            all_nutrients_consumed = np.hstack((all_nutrients_consumed, output_dict[run]['nutrients_consumed'].flatten()))
        else:
            ...
            all_biomass = np.vstack((all_biomass, output_dict[run]['total_biomass'].flatten()))
            all_longest_branch = np.vstack((all_longest_branch, output_dict[run]['longest_branch_length'].flatten()))
            all_nutrients_consumed = np.vstack((all_nutrients_consumed, output_dict[run]['nutrients_consumed'].flatten()))
    ...
    avg_biomass = np.average(all_biomass, axis=0)
    avg_longest_branch = np.average(all_longest_branch, axis=0)
    avg_nutrients_consumed = np.average(all_nutrients_consumed, axis=0)
    ...
    std_biomass = np.std(all_biomass, axis=0)
    std_longest_branch = np.std(all_longest_branch, axis=0)
    ...
    # Plot average biomass with error bars
    hf.plot_errorbar_biomass(output_dict[0]['array_times'].flatten(), 
                            avg_biomass, std_biomass, 
                            folder_string, param_string, params, num_runs)
    
    # Plot biomass comparison across runs
    all_times = [np.array(output_dict[i]['array_times']) for i in range(num_runs)]
    all_biomass_arrays = [np.array(output_dict[i]['total_biomass']) for i in range(num_runs)]
    run_labels = [f'Run {i+1}' for i in range(num_runs)]
    
    hf.plot_biomass_comparison(all_times, all_biomass_arrays, run_labels,
                              folder_string, param_string, params)
    
    # Plot average longest branch with error bars
    hf.plot_errorbar_longest_branch(output_dict[0]['array_times'].flatten(),
                                    avg_longest_branch, std_longest_branch,
                                    folder_string, param_string, params, num_runs)
    
    # Plot longest branch comparison across runs
    all_longest_arrays = [np.array(output_dict[i]['longest_branch_length']) for i in range(num_runs)]
    hf.plot_longest_branch_comparison(all_times, all_longest_arrays, run_labels,
                                     folder_string, param_string, params)
    
    # Plot biomass vs nutrients with error bars
    hf.plot_errorbar_biomass_vs_nutrients(avg_nutrients_consumed, 
                                         avg_biomass, std_biomass,
                                         folder_string, param_string, 
                                         params, num_runs)
    
    # Plot biomass vs nutrients comparison
    all_biomass_nutrients = [np.array(output_dict[i]['total_biomass']) for i in range(num_runs)]
    all_nutrients_arrays = [np.array(output_dict[i]['nutrients_consumed']) for i in range(num_runs)]
    hf.plot_biomass_vs_nutrients_comparison(all_biomass_nutrients,
                                           all_nutrients_arrays,
                                           run_labels, folder_string, 
                                           param_string, params)
    
    # Plot longest branch vs nutrients with error bars
    hf.plot_errorbar_longest_branch_vs_nutrients(avg_nutrients_consumed,
                                                 avg_longest_branch,
                                                 std_longest_branch,
                                                 folder_string, param_string,
                                                 params, num_runs)
    
    # Plot longest branch vs nutrients comparison
    hf.plot_longest_branch_vs_nutrients_comparison(all_longest_arrays,
                                                   all_nutrients_arrays,
                                                   run_labels, folder_string,
                                                   param_string, params)
