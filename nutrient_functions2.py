...
#Here is a new diffusion method using finite volume:
def finite_volume_diffusion_2d(sub_e, time_step, params, glucose_mask):
    """
    Mass-conserving finite volume explicit diffusion for irregular domains.
    Only updates where glucose_mask is True.
    No-flux at mask boundaries.
    
    Parameters:
        sub_e: 2D numpy array, concentrations
        time_step: float
        params: dict with 'diffusion_e_gluc' (D) and 'dy' (grid size)
        glucose_mask: 2D boolean array, True where diffusion occurs
    
    Returns:
        sub_e_new: 2D numpy array, updated concentrations
    """
    D = params['diffusion_e_gluc']
    dy = params['dy']
    r = D * time_step / dy**2
    rows, cols = sub_e.shape

        # Total glucose before diffusion
    glucose_before = np.sum(sub_e[glucose_mask])

    sub_e_new = sub_e.copy()

    # For each cell, compute net flux from each neighbor (only if both are in mask)
    for i in range(rows):
        for j in range(cols):
            if not glucose_mask[i, j]:
                continue  # skip outside

            # For each neighbor (up, down, left, right)
            for di, dj in [(-1,0), (1,0), (0,-1), (0,1)]:
                ni, nj = i + di, j + dj
                if 0 <= ni < rows and 0 <= nj < cols and glucose_mask[ni, nj]:
                    # Compute flux: from neighbor to (i,j)
                    flux = r * (sub_e[ni, nj] - sub_e[i, j])
                    # Add half the flux to each (symmetric update)
                    sub_e_new[i, j] += 0.5 * flux
                    sub_e_new[ni, nj] -= 0.5 * flux
    
    # Total glucose after diffusion
    glucose_after = np.sum(sub_e_new[glucose_mask])

    print(f"[diffusion] Mass before: {glucose_before:.6g}, after: {np.sum(sub_e_new[glucose_mask]):.6g}, "
          f"used: {glucose_before-glucose_after:.6g}", f"percentage change: {100 * (glucose_before - glucose_after) / glucose_before:.6g}")
    return sub_e_new
# ----------------------------------------------------------------------------
...
def transloc(mycelia, params, num_total_segs, dtt, isActiveTrans, whichInitialCondition,
    ...
    Purpose
    -------
    Calculate the change in nutrients due to translocation (diffusion, advection,
    ...
    for idx in range(num_total_segs):
        ...
        # Advection is to the closest tip. Assume that diffusion is too. Find the immediate neighbors that are closer to tips.
    ...
    #%% Advection (glucose and trehalose) and Active Transport (cell wall materials):
    ...
    cw_curr_mod = cw_curr_concentrations #This is the amount of cell wall material present (after metabolism made more)
    ...
    # Rate/velocity of active transport and advection. The units here are concentraton/sec
    active_trsprt_vel_cw = params['active_trsprt_vel_cw']
    advection_rate = active_trsprt_vel_cw # Advection and active transport rates are set the same.
    advection_vel_gluc = advection_rate*params['yield_c']
    
    K_cw = active_trsprt_vel_cw*params['dt_i'] #K_cw is now the maximum amount of material that can be transported in dt.
    ...
    gluc_advection_term = np.zeros((num_total_segs,1))
    treha_advection_term = np.zeros((num_total_segs,1))

    # Advection and active transport are  to the closest tip. The to_nbr and from_nbr lists are calculated above in the diffusion section.
    ...
    for idx in range(num_total_segs):
        ...
        if (len(from_nbrs_idx) + len(to_nbrs_idx) >0):
            ...
            #cw_transport_term[idx] = active_trsprt_vel_cw* \
            ...
            gluc_advection_term[idx] = advection_vel_gluc* \
    ...
    print('Net cell wl active transport',np.sum(cw_transport_term[:num_total_segs]))
    print('Mean cell wl difference',np.mean(np.abs(cw_delta_count[:num_total_segs])))
    print('Mean cell wl active transport',np.mean(np.abs(cw_transport_term[:num_total_segs])))
    ...
    if (np.any(mycelia['cw_i'][:num_total_segs] + params['dt_i']*cw_transport_term < 0)):
        bad_idx = np.where((mycelia['cw_i'][:num_total_segs] + params['dt_i']*cw_transport_term[:num_total_segs]) < 0)
        print('Bad indices: ',bad_idx)
        print('Cell Wall before conversion:',mycelia['cw_i'][bad_idx])
        print('Amount converted:',cw_transport_term[bad_idx]*params['dt_i'])
        print('Convert rate:',cw_transport_term[bad_idx])
        print('Cell Wall after conversion:',mycelia['cw_i'][bad_idx] + params['dt_i']*(cw_transport_term[bad_idx]))
        #breakpoint()
       # Update concentrations due to advection
    if (np.any(mycelia['gluc_i'][:num_total_segs] + params['dt_i']*gluc_advection_term < 0)):
        bad_idx = np.where((mycelia['gluc_i'][:num_total_segs] + params['dt_i']*gluc_advection_term[:num_total_segs]) < 0)
        print('Bad indices: ',bad_idx)
        print('Glucose before conversion:',mycelia['gluc_i'][bad_idx])
        print('Amount converted:',gluc_advection_term[bad_idx]*params['dt_i'])
        print('Convert rate:',gluc_advection_term[bad_idx])
        print('Glucose after conversion:',mycelia['gluc_i'][bad_idx] + params['dt_i']*(gluc_advection_term[bad_idx]))
        #breakpoint()

    mycelia['cw_i'][:num_total_segs] += params['dt_i']*(cw_transport_term)
    mycelia['gluc_i'][:num_total_segs] += params['dt_i']*(gluc_advection_term)
...
def uptake(sub_e_gluc, mycelia, num_total_segs, var_nutrient_backgrnd, time_step, params):
    """
    Try to fix the glucose uptake from the external grid to the internal mycelium,
    ensuring mass conservation by processing uptake on a per-grid-cell basis.
    ...
    num_total_segs : int
        Current total number of segments in the mycelium.
    var_nutrient_backgrnd : float
        A factor to scale the amount of glucose removed from the external grid.
    time_step : float
    ...
    mycelia : dictionary
        Updated structural information of mycelia colony for all hyphal segments.
    """
    # Store initial totals for conservation check
    glucose_before_external = np.sum(sub_e_gluc)
    gluc_i_before_internal = np.sum(mycelia['gluc_i'][:num_total_segs])

    # Get segment locations on the external grid
    xy_e_idx_og = mycelia['xy_e_idx'][:num_total_segs, :].astype(int)
    
    # Calculate potential glucose uptake for each segment individually
    gluc_e_at_segments = sub_e_gluc[tuple(np.transpose(xy_e_idx_og))].copy()
    relative_seg_vol = mycelia['seg_vol'][:num_total_segs].flatten() / params['init_vol_seg']
    
    # Calculate potential uptake rate
    potential_uptake = np.zeros_like(relative_seg_vol)
    active_segs = np.where(relative_seg_vol > 0)[0]
    
    potential_uptake[active_segs] = time_step * michaelis_menten(
        params['ku1_gluc'],
        params['Ku2_gluc'] / relative_seg_vol[active_segs],
        gluc_e_at_segments[active_segs]
    )

    # Inactivate uptake for segments with zero length or negative branch ID
    seg_lengths = mycelia['seg_length'][:num_total_segs]
    potential_uptake[np.where(seg_lengths * seg_lengths < 0.1 * params['diffusion_i_gluc'])[0]] = 0.0
    potential_uptake[np.where(mycelia['branch_id'][:num_total_segs] < 0)[0]] = 0.0

    # Process uptake on a per-grid-cell basis to avoid multiple subtractions
    unique_cells, inverse_indices = np.unique(xy_e_idx_og, axis=0, return_inverse=True)
    
    final_uptake = np.zeros_like(potential_uptake)

    for i, cell_coords in enumerate(unique_cells):
        # Find all segments located in this unique grid cell
        segments_in_cell_indices = np.where(inverse_indices == i)[0]
        
        # Calculate the total glucose demand from all segments in this cell
        total_demand_in_cell = np.sum(potential_uptake[segments_in_cell_indices])
        
        # Get the available glucose in this cell
        available_glucose = sub_e_gluc[cell_coords[0], cell_coords[1]]
        
        if total_demand_in_cell > available_glucose:
            # If demand exceeds supply, scale down the uptake for each segment in this cell
            scaling_factor = available_glucose / total_demand_in_cell
            adjusted_uptake = potential_uptake[segments_in_cell_indices] * scaling_factor
            final_uptake[segments_in_cell_indices] = adjusted_uptake
            
            # The total glucose removed from the cell is the total available glucose
            glucose_removed_from_cell = available_glucose
        else:
            # If supply is sufficient, the uptake is as calculated
            final_uptake[segments_in_cell_indices] = potential_uptake[segments_in_cell_indices]
            glucose_removed_from_cell = total_demand_in_cell
            
        # Update the external grid cell. This happens only ONCE per unique cell.
        sub_e_gluc[cell_coords[0], cell_coords[1]] -= glucose_removed_from_cell * var_nutrient_backgrnd

    # Update the internal glucose for all segments based on the final, adjusted uptake
    mycelia['gluc_i'][:num_total_segs] += params['yield_u'] * final_uptake.reshape(-1, 1)

    # Final conservation check
    glucose_after_external = np.sum(sub_e_gluc)
    gluc_i_after_internal = np.sum(mycelia['gluc_i'][:num_total_segs])
    
    glucose_change_external = glucose_before_external - glucose_after_external
    glucose_change_internal = gluc_i_after_internal - gluc_i_before_internal
    
    print(f"[4. uptake] Total external glucose before: {glucose_before_external:.6g}, after: {glucose_after_external:.6g}, used: {glucose_change_external:.6g}")
    print(f"[4. Internal] Total internal glucose before: {gluc_i_before_internal:.6g}, after: {gluc_i_after_internal:.6g}, gained: {glucose_change_internal:.6g}")
    
    # This check should now be zero
    conservation_check = (glucose_change_external * params['yield_u']) - glucose_change_internal
    print(f"[4. Conservation Check] (External Change * Yield) - Internal Change: {conservation_check:.6g}")
